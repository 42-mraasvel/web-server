...
*/
int ChunkedParser::parse(std::string const & buffer, std::size_t & index)
{
	...
	{
		if ((this->*state_dispatch[_state])(buffer, index) == ERR)
		{
			return setError();
		}
	}
...
*/
int ChunkedParser::parseSize(std::string const & buffer, std::size_t & index)
{
	std::size_t start = index;
	util::skip(buffer, index, isHex);
	...
	if (util::strtoul(_leftover, _chunk_size, 16) == ERR
		|| util::additionOverflow(_chunk_size, message_body.size()))
	{
		return ERR;
	}
...
int ChunkedParser::parseData(std::string const & buffer, std::size_t & index)
{
	if (message_body.size() + _chunk_size > _max_size)
	{
	...
	if (_content_parser.isComplete())
	{
		_next_state = SIZE;
		_state = ENDLINE;
		message_body.append(_content_parser.getContent());
		_content_parser.reset();
	}
	return OK;
}

int ChunkedParser::parseTrailer(std::string const & buffer, std::size_t & index)
{
...
int ChunkedParser::parseEndLine(std::string const & buffer, std::size_t & index)
{
...
*/
int ChunkedParser::parseDiscardLine(std::string const & buffer, std::size_t & index)
{
	std::size_t start = index;
	index = util::findEndLine(_leftover, buffer, index);
...
int ChunkedParser::appendMessageBody(std::string& dest) {
	if (dest.size() == 0) {
		dest.swap(message_body);
	} else {
		dest.append(message_body);
		message_body.clear();
	}
	return OK;
}

int ChunkedParser::appendHeader(HeaderField& dest) {
